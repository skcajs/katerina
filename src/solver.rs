...
#[allow(dead_code)]
fn derivatives(incoming: SVector<f64, 8>, a: f64) -> SVector<f64, 8> {
    let t = incoming[0];
    let r = incoming[1];
    let theta = incoming[2];
    let phi = incoming[3];
    ...
    let u3_dot = -2.0
        ...
        * (-a * r * (-4.0 * r2 / s2 + 2.0 / (s))
            / (2.0 * a2 * r * sin_theta2 - 2.0 * a2 * r - 2.0 * r3
                ...
                + a2 * r2
                + r4)
            + 0.5 * (4.0 * a * r2 * sin_theta2 / s2 - 2.0 * a * sin_theta2 / (s)) * (-2.0 * r + s)
...
mod tests {
    ...
    fn test_kerr() {
        ...
        let result2: SVector<f64, 8> = derivatives(incoming, metric.a);
        println!("{:?}", result);
        println!("{:?}", result2);
    }
